...
export class IntervalPipe implements PipeTransform {
  ...
  private buildLongString(interval: Interval, precision: Precision) {
    ...
    if (precision >= Precision.hours) ret.push(this.pluralize('hour', hours));
  ...
  private buildShortString(interval: Interval, precision: Precision) {
    ...
    if (precision >= Precision.minutes) ret.push(this.formatNumber(minutes));
    if (precision >= Precision.seconds) ret.push(this.formatNumber(seconds));
    return ret.join(':');
  }
